...
_DAY_OFFSETS = [2, 4, 6]      # jours après « maintenant » (étalés dans la semaine)
_DEFAULT_HOUR = 18            # le soir performe le mieux par défaut
_N_POSTS = 3                  # nombre de publications planifiées
...
def build_weekly_plan(now: Optional[dt.datetime] = None) -> Optional[dict]:
    ...
    for i, g in enumerate(chosen):
        ...
        offset = _DAY_OFFSETS[i] if i < len(_DAY_OFFSETS) else (2 * i + 2)
        when = (now + dt.timedelta(days=offset)).replace(
            hour=hour, minute=0, second=0, microsecond=0)
        fmt = "carousel" if i == 0 else "simple"   # 1re galerie en carrousel (saves)
...
def _render(now: dt.datetime, items: List[dict], perf: dict) -> str:
    ...
    lines.append("")
    icons = {"carousel": "🖼️ Carrousel", "simple": "📷 Post simple", "reel": "🎬 Reel"}
    for it in items:
        d = _FR_DAYS[it["when"].weekday()]
        lines.append(f"• *{d.capitalize()} {it['when'].hour}h* — {icons.get(it['fmt'], it['fmt'])} "
                     f"*{it['nom']}*")
        lines.append(f"   _{it['reason']}_")
    lines.append("")
    lines.append("💡 _Pense à un Reel dans la semaine (le format qui fait le plus de portée)._")
    return "\n".join(lines)
