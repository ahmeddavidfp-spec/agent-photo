...
def publish_to_threads(image_url, caption):
    ...
    # Sécurité : On s'assure que l'URL est propre pour Threads
...
def generate_ai_caption(image_url, galerie_nom):
    ...
    STRATÉGIE SEO : 1. Crochet (Hook) percutant en ligne 1. 2. Vocabulaire riche. 3. 2-3 phrases descriptives.
    Série complète sur : {galerie_link}
    5 hashtags variés {extra_tag}.
    STRICT : PAS de gras (**), PAS de ###, PAS de majuscules intégrales."""
...
def telegram_webhook():
    data = request.json
    token = os.environ.get('TELEGRAM_TOKEN')
    if not data or "callback_query" not in data and "message" not in data: return jsonify({"status": "ok"})
    ...
    if "message" in data:
        ...
        if action == "view_stats":
            requests.post(f"https://api.telegram.org/bot{token}/sendMessage", json={"chat_id": chat_id, "text": get_latest_insights()})
